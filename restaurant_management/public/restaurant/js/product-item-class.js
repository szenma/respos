class ProductItem {
				...
				<tr>
					<td style="max-height: 60px;">
						<div class="fields row">
                            ...
                            <div class="waiter-field col-md-3">
							</div>
							<div class="item-group-field col-md-3">
							</div>
						</div>
					</td>
				</tr>
    ...
    make_fields() {
        ...
        this.waiter_field = frappe.ui.form.make_control({
            df: {
                fieldtype: 'Link',
                label: 'Waiter',
                options: 'User',
            ...
            parent: this.wrapper.find('.waiter-field'),
            render_input: true,
        });
    ...
    get_items({ start = 0, page_length = 40, search_value = '', item_group = this.parent_item_group } = {}) {
        ...
        return new Promise(res => {
            frappe.call({
                ...
                freeze: true,
                args: { start, page_length, price_list, item_group, search_value, pos_profile,item_cod }
            }).then(r => {
                res(r.message);
            });
        });
    ...
    get_item_html(item) {
        ...
        var start=0;
        var page_length=40;
        var item_group=this.parent_item_group;
        var search_value='';
        var item_cod=item.item_code
            frappe.call({
                method: "erpnext.selling.page.point_of_sale.point_of_sale.get_items",
                freeze: true,
                args: { start, page_length, price_list, item_group , search_value, pos_profile,item_cod }
